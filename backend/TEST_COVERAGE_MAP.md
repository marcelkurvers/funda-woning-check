# Test Coverage Map

**Generated**: 2025-12-15 15:10:03 UTC
**Project**: `/Users/marcelkurvers/Development/funda-app/ai-woning-rapport-WERKEND-local/backend`

## Executive Summary

**Status**: ✅ All tests passing
**Success Rate**: 100.0% (1/1 test suites)

- ✅ **Passed**: 1
- ❌ **Failed**: 0
- ⏭️ **Skipped**: 5
- 📊 **Total**: 6

## Language Detection

**Detected Languages**: 2/6

### ✓ Python
- **Source files**: 29
- **Test files**: 17
- **Test ratio**: 59% (17 test files per 29 source files)

### ✓ Javascript/Typescript
- **Source files**: 1
- **Test files**: 0

### ○ Go
- **Source files**: 0
- **Test files**: 0
- **Note**: No Go *.go files detected

### ○ Rust
- **Source files**: 0
- **Test files**: 0
- **Note**: No Rust *.rs files detected

### ○ Java
- **Source files**: 0
- **Test files**: 0
- **Note**: No Java *.java files detected

### ○ Ruby
- **Source files**: 0
- **Test files**: 0
- **Note**: No Ruby *.rb files detected

## Test Execution Results

### ✅ Successful Test Suites

| Framework | Tests | Duration | Details |
|-----------|-------|----------|---------|
| pytest | N/A | N/A | ✓ All tests passed |

### ⏭️ Skipped Test Suites

| Framework | Reason |
|-----------|--------|
| jest | Detected 1 JS/TS source files but no test files to execute. |
| go test | No Go source files detected in repository. |
| cargo test | No Rust source files detected in repository. |
| maven/gradle | No Java source files detected in repository. |
| rspec/minitest | No Ruby source files detected in repository. |
