...
Uses REAL providers with API keys from environment variables.
"""
import pytest
import os
import sys
...
class TestProviderFactory:
    ...
    def test_create_openai_provider_with_api_key(self):
        ...
        api_key = os.getenv("OPENAI_API_KEY") or "test-key-for-init"
    ...
    def test_create_anthropic_provider_with_api_key(self):
        ...
        api_key = os.getenv("ANTHROPIC_API_KEY") or "test-key-for-init"
    ...
    def test_create_anthropic_provider_without_api_key_raises_error(self):
        ...
        old_ai_key = os.environ.pop("AI_ANTHROPIC_API_KEY", None)
        ...
        try:
            ...
            if old_ai_key:
                os.environ["AI_ANTHROPIC_API_KEY"] = old_ai_key
    ...
    def test_create_gemini_provider_with_api_key(self):
        ...
        api_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY") or "test-key-for-init"
    ...
    def test_create_gemini_provider_without_api_key_raises_error(self):
        ...
        old_ai_gemini = os.environ.pop("AI_GEMINI_API_KEY", None)
        ...
        try:
            ...
            if old_ai_gemini:
                os.environ["AI_GEMINI_API_KEY"] = old_ai_gemini
    ...
    def test_create_all_available_providers_with_valid_config(self):
        """Test that all available providers can be created with valid configuration"""
        ...
        if OPENAI_AVAILABLE:
            ...
            openai = ProviderFactory.create_provider("openai", api_key=api_key)
            assert isinstance(openai, OpenAIProvider)

        # Anthropic (needs API key)
        if ANTHROPIC_AVAILABLE:
            api_key = os.getenv("ANTHROPIC_API_KEY") or "test-key"
            anthropic = ProviderFactory.create_provider("anthropic", api_key=api_key)
            assert isinstance(anthropic, AnthropicProvider)

        # Gemini (needs API key)
        if GEMINI_AVAILABLE:
            api_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY") or "test-key"
            gemini = ProviderFactory.create_provider("gemini", api_key=api_key)
    ...
    def test_provider_factory_respects_timeout_parameter(self):
        ...
        if OPENAI_AVAILABLE:
            api_key = os.getenv("OPENAI_API_KEY") or "test-key"
            openai = ProviderFactory.create_provider("openai", api_key=api_key, timeout=custom_timeout)
            assert openai.timeout == custom_timeout

        if ANTHROPIC_AVAILABLE:
            api_key = os.getenv("ANTHROPIC_API_KEY") or "test-key"
            anthropic = ProviderFactory.create_provider("anthropic", api_key=api_key, timeout=custom_timeout)
            assert anthropic.timeout == custom_timeout

        if GEMINI_AVAILABLE:
            api_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY") or "test-key"
            gemini = ProviderFactory.create_provider("gemini", api_key=api_key, timeout=custom_timeout)
    ...
    def test_all_registered_providers_can_be_created(self):
        ...
        for provider_name in providers:
            ...
            if provider_name != "ollama":
                # Use env var or test key
                if provider_name == "openai":
                    kwargs["api_key"] = os.getenv("OPENAI_API_KEY") or "test-key"
                elif provider_name == "anthropic":
                    kwargs["api_key"] = os.getenv("ANTHROPIC_API_KEY") or "test-key"
                elif provider_name == "gemini":
                    kwargs["api_key"] = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY") or "test-key"
