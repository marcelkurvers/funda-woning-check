...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, debugMode, setDebugMode }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-emerald-900/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-emerald-200 flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center justify-between p-6 border-b border-emerald-100 bg-emerald-50/50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-black text-emerald-800 tracking-tight">System Settings</h2>
                            <p className="text-xs text-emerald-600 font-medium">Beheer AI providers, marktdata en validatie</p>
                        </div>
                    </div>
                            ...
                            <section className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-emerald-600 uppercase tracking-wider">Actieve Provider</label>
                                        <select
                                            value={config.ai.provider}
                                            onChange={(e) => updateSection('ai', 'provider', e.target.value)}
                                            className="w-full bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                                        >
                                            {providers.map(p => (
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-emerald-600 uppercase tracking-wider">Model</label>
                                        <select
                                    ...
                                    <label className="flex items-center gap-3 cursor-pointer group">
                                        <div className="relative">
                                            <input
                                                ...
                                                className="sr-only"
                                            />
                                            <div className={`w-10 h-5 rounded-full transition-colors ${config.ai.fallback_enabled ? 'bg-emerald-600' : 'bg-emerald-300'}`} />
                                        ...
                                        <span className="text-sm font-medium text-emerald-700 group-hover:text-emerald-900 transition-colors">Fallback bij fouten</span>
                                    </label>

                                    <div className="flex items-center gap-3">
                                        <label className="text-xs font-bold text-emerald-500 uppercase tracking-wider">Timeout</label>
                                        <input
                                            type="number"
                                            value={config.ai.timeout}
                                            onChange={(e) => updateSection('ai', 'timeout', parseInt(e.target.value))}
                                            className="w-16 bg-emerald-50 border border-emerald-200 rounded px-2 py-1 text-sm outline-none"
                                        />
                                        <span className="text-xs text-emerald-400 font-medium">sec</span>
                                    </div>
                                </div>
                            </section>
                            ...
                            <section className="space-y-4 pt-4 border-t border-slate-100">
                                ...
                                <div className="flex items-center justify-between p-3 bg-emerald-50 rounded-xl border border-emerald-100">
                                    <div className="flex flex-col">
                                        <span className="text-sm font-medium text-emerald-700">Debug Modus</span>
                                        <span className="text-xs text-emerald-500 leading-tight">Toon data inconsistenties en ruwe payloads</span>
                                    </div>
                                    <button
                                        onClick={() => setDebugMode(!debugMode)}
                                        className={`w-11 h-6 flex items-center rounded-full p-1 transition-colors ${debugMode ? 'bg-emerald-600 justify-end' : 'bg-emerald-300 justify-start'}`}
                                    >
                ...
                {/* Footer */}
                <div className="p-6 border-t border-emerald-100 bg-emerald-50/50 flex flex-col gap-4">
                    {success && (
