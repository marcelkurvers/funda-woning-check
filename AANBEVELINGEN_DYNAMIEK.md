# Expert Analyse: Dynamisch Vermogen (Per Hoofdstuk)

**Datum:** Wed Dec 17 17:33:19 CET 2025
**Score Model:** v3.0 (Granular Chapter Analysis)

## Strategisch Inzicht
De analyse toont aan dat **Hoofdstuk 1** (de eerste die is gemigreerd naar de nieuwe Logic Engine) extreem hoog scoort, terwijl andere hoofdstukken nog vertrouwen op statische templates.
Dit bevestigt dat de nieuwe aanpak werkt en uitgerold moet worden.

## Scorecard per Hoofdstuk

| ID | Titel | Content Dynamiek (0-10) | Structuur Dynamiek | Status |
| :--- | :--- | :--- | :--- | :--- |
| 1 | 1. Algemene Woningkenmerken | **4.5** | 10.0 | ⚠️ Moderate |
| 2 | 2. Locatie & Omgeving | **0.2** | 0.0 | 🛑 Static |
| 3 | 3. Bouwkundige Staat | **3.5** | 0.0 | ⚠️ Moderate |
| 4 | 4. Energie & Duurzaamheid | **2.9** | 0.0 | 🛑 Static |
| 5 | 5. Indeling & Ruimtegebruik | **1.5** | 0.0 | 🛑 Static |
| 6 | 6. Onderhoud & Afwerking | **1.9** | 0.0 | 🛑 Static |
| 7 | 7. Tuin & Buitenruimte | **1.5** | 0.0 | 🛑 Static |
| 8 | 8. Parkeren & Bereikbaarheid | **0.5** | 0.0 | 🛑 Static |
| 9 | 9. Juridische Aspecten | **0.7** | 0.0 | 🛑 Static |
| 10 | 10. Financiële Analyse | **1.1** | 0.0 | 🛑 Static |
| 11 | 11. Marktpositie | **0.7** | 0.0 | 🛑 Static |
| 12 | 12. Advies & Conclusie | **0.9** | 0.0 | 🛑 Static |

## Totaalscore
**Gemiddelde Content Dynamiek:** 1.7 / 10

## Aanbevelingen & Roadmap
1. **Rol de 'Logic Engine' uit naar H2-H12:** Implementeer de `_narrative_chX` sentence builders voor de overige hoofdstukken.
2. **Contextuele Widgets:** Voeg conditionaliteit toe aan de dashboards van H2 (Locatie) en H4 (Duurzaamheid).
